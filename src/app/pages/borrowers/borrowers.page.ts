...
import {
  ...
  ModalController,
  ToastController,
  AlertController, IonSkeletonText, IonItem, IonLabel, IonCard, IonCardHeader, IonCardContent, IonCardTitle, IonList } from '@ionic/angular/standalone';
...
@Component({
  ...
  standalone: true,
  imports: [IonList, IonCardTitle, IonCardContent, IonCardHeader, IonCard, IonItem, IonSkeletonText, 
    IonRow,
    CommonModule,
    FormsModule,
    NgxDatatableModule,
...
export class BorrowersPage implements OnInit, AfterViewInit {
  ...
  tableColumns: any[] = [
    ...
    { prop: 'contact_no_borrower', name: 'Contact #', width: 150 },
    {
      ...
      width: 150,
      pipe: {
        transform: (value: string) => this.datePipe.transform(value, 'short'),
      },
    },
    {
      name: 'Actions',
  ...
  tryAssignTemplateAndData() {
    if (this.actionsTemplate && !this.actionsTemplateAssigned) {
      const actionsCol = this.tableColumns.find(
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: async () => {
            ...
            try {
              const response = await this.borrowerService.deleteBorrower(
                borrower.id
              );
